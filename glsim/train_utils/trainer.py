...
class Trainer():
    def __init__(self, args, model, criterion, optimizer, lr_scheduler,
        ...
        tb_log_dir = osp.join(args.results_dir, 'tensorboard')
    ...
    def predict(self, images, targets, train=True):
        ...
        with self.amp_autocast():
            if self.args.model_name == 'cal':
                ...
                if (self.args.save_images and train and (self.curr_iter % self.args.save_images == 0)):
                    ...
                    if not self.args.debugging and not self.args.vis_errors and not self.args.test_offline and wandb.run is not None and not getattr(wandb.run, 'disabled', True):
                        wandb.log({'crops': wandb.Image(images_crops[0])})
                    self.saved = True
                return output, loss
        ...
        if self.args.anchor_size and crops:
            ...
            self.save_images(images, osp.join(self.args.results_dir, f'test.png'), image_size)
            if not self.args.debugging and not self.args.vis_errors and not self.args.test_offline and wandb.run is not None and not getattr(wandb.run, 'disabled', True):
                wandb.log({'crops': wandb.Image(images[0])})
            self.saved = True
    ...
    def train_epoch(self):
        ...
        if self.args.local_rank == 0:
            ...
            lr_curr = self.optimizer.param_groups[0]['lr']
            self.writer.add_scalar('train/acc1', top1.avg, self.epoch)
            self.writer.add_scalar('train/acc5', top5.avg, self.epoch)
            self.writer.add_scalar('train/loss', losses.avg, self.epoch)
            self.writer.add_scalar('train/lr', lr_curr, self.epoch)
    ...
    def validate_epoch(self, val_loader):
        ...
        if self.args.local_rank == 0:
            print(' * Acc@1 {top1.avg:.3f} Acc@5 {top5.avg:.3f}'.format(top1=top1, top5=top5))
            self.writer.add_scalar('val/acc1', top1.avg, self.epoch)
            self.writer.add_scalar('val/acc5', top5.avg, self.epoch)
            self.writer.add_scalar('val/loss', losses.avg, self.epoch)
    ...
    def epoch_end_routine(self, train_acc, train_loss, val_acc, val_loss):
        ...
        if wandb.run is not None and not getattr(wandb.run, 'disabled', True):
            wandb.log(log_dic)
